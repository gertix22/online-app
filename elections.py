...
df = pd.read_csv(
    "https://raw.githubusercontent.com/gertix22/online-app/main/politics.csv"
)
...
for x in ["AZ", "FL", "GA", "IA", "ME", "MI", "NC", "NV", "OH", "PA", "TX", "WI"]:
    input_list.append(Input(component_id=f"radiolist-{x}", component_property="value"))
...
app.layout = html.Div(
    [
        dbc.Row(
            [
                dbc.Col(
                    html.H1("USA Elections 2020", style={"textAlign": "center"}),
                    width=12,
                )
            ]
        ),
        dbc.Row(
            [
                ...
                dbc.Col(
                    dcc.Graph(
                        id="my-choropleth", figure={}, config={"displayModeBar": False}
                    ),
                    xs=8,
                    sm=8,
                    md=8,
                ...
                dbc.Col(
                    dcc.Graph(id="my-bar", figure={}, config={"displayModeBar": False}),
                    xs=6,
                    sm=6,
                    md=6,
                    lg=4,
...
def update_graph(az, fl, ga, ia, me, mi, nc, nv, oh, pa, tx, wi):
    ...
    fig_map = px.choropleth(
        dff,
        locations="state",
        hover_name="electoral votes",
        locationmode="USA-states",
        color="party",
        scope="usa",
        color_discrete_map={"democrat": "blue", "republican": "red", "unsure": "grey"},
    )
